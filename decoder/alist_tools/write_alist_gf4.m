...
sum_dv = full(sum(H~=0,1));
sum_dc = full(sum(H~=0,2));
...
vals = zeros(M,max(sum_dc));
for k=1:M
    temp = H(k,:);
    vals(k,1:sum(temp~=0)) = temp(temp~=0);
end

dlmwrite(filename, vals,  '-append', 'delimiter', ' ', 'precision','%d');

vals = zeros(N,max(sum_dv));
Hp = H';
for k=1:N
    temp = Hp(k,:);
    vals(k,1:sum(temp~=0)) = temp(temp~=0);
